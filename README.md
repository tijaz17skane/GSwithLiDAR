# 3D Gaussian Splatting with LiDAR Data
...
<summary><span style="font-weight: bold;">Command Line Arguments for train.py</span></summary>
  ...
  #### --use_graph_maha
  Enable/disable graph Mahalanobis loss computation. Set to ```False``` to completely skip the graph Mahalanobis loss calculation during training, which can improve performance. ```True``` by default.
...
#### Test Set Selection
When using ```--eval```, the code supports two methods for selecting test cameras:

1. **Custom test.txt file (recommended)**: Create a file at ```<source_path>/sparse/0/test.txt``` containing one camera/image name per line. The camera names must exactly match the image names in your COLMAP ```images.bin``` or ```images.txt``` file. If this file exists, it will be used and the test cameras will be printed at the start of training.

2. **LLFF holdout (automatic)**: If no ```test.txt``` file is found and your dataset path contains "360", the code will automatically use the LLFF holdout method (every 8th camera, sorted by name).

During training, the code will display:
- Which test cameras were loaded from ```test.txt``` or selected via LLFF holdout
- Which test cameras were successfully found in the COLMAP data (marked with ✓)
- Which test cameras from ```test.txt``` were not found in COLMAP data (marked with ✗) - this helps identify mismatches

Example output:
```
Reading test cameras from test.txt (4 total):
  - front_20220920_DTAG_T4_2292003_1891.jpg
  - left_20220920_DTAG_T4_2292003_1896.jpg
  ...
Found 4 test cameras in COLMAP data:
  ✓ front_20220920_DTAG_T4_2292003_1891.jpg
  ✓ left_20220920_DTAG_T4_2292003_1896.jpg
  ...
```

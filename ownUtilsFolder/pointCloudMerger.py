...
def merge_point_clouds(points1, colors1, errors1, ids1, tracks1,
    ...
    Reassign point IDs to avoid conflicts.
    """
    # Reassign IDs sequentially starting from 1
    n1 = len(points1)
    n2 = len(points2)
    
    merged_ids = list(range(1, n1 + n2 + 1))
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
OUTPUTS:
  points3Dcombined.txt - All points from both inputs (COLMAP format)
  ...
  points3DcombinedCropped.ply - All LiDAR points + COLMAP points within LiDAR bounding box (PLY format)
    ...
    # Output 1: Combined point cloud (all points)
    print("\n" + "-"*70)
    ...
    write_colmap_points3D(output_combined, combined_points, combined_colors, 
                          combined_errors, combined_ids, combined_tracks, header_colmap)
    print(f"  ✓ Saved {len(combined_points)} points")
    print(f"    - {len(points_colmap)} from COLMAP")
    print(f"    - {len(points_lidar)} from LiDAR")
    ...
    print(f"  ✓ Saved PLY with {len(combined_points)} points")
    
    # Output 2: Cropped combined point cloud (COLMAP points within LiDAR bbox + all LiDAR)
    print("\n" + "-"*70)
    print("OUTPUT 2: Cropped combined point cloud (COLMAP within LiDAR bbox + all LiDAR)")
    print("-"*70)
    
    print("Filtering COLMAP points by LiDAR bounding box...")
    ...
    print(f"    ({len(points_colmap) - len(points_colmap_filtered)} points removed)")
    ...
    write_colmap_points3D(output_cropped, cropped_points, cropped_colors,
                          cropped_errors, cropped_ids, cropped_tracks, header_colmap)
    print(f"  ✓ Saved {len(cropped_points)} points")
    print(f"    - {len(points_colmap_filtered)} from COLMAP (cropped)")
    print(f"    - {len(points_lidar)} from LiDAR (all)")
    ...
    print(f"  - points3Dcombined.txt ({len(combined_points)} points)")

...
def main():
    ...
    for img in meta['images']:
        ...
        t_cam, R_cam , q_cam = world2cam(qw, qx, qy, qz, t_norm[0], t_norm[1], t_norm[2])
        tx_cam, ty_cam, tz_cam = t_cam.tolist()
        qw_cam, qx_cam, qy_cam, qz_cam = q_cam.tolist()

        # Store for images.txt
        image_data.append({
            'id': len(image_data) + 1,
            'qw': qw_cam, 'qx': qx_cam, 'qy': qy_cam, 'qz': qz_cam,
            'tx': tx_cam, 'ty': ty_cam, 'tz': tz_cam,
            'cam_id': cam_id,
            'name': img_name
        })
        processed_count += 1

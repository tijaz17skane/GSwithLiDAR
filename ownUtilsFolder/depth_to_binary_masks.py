...
def main():
    ...
    for depth_path in tqdm(depth_files, desc="Processing depth images"):
        ...
        # Naming: remove an internal .jpg before final .png if present like image_name.jpg.png
        # We'll take original stem and original suffix; if filename has pattern *.jpg.png we simplify.
        original_name = depth_path.name
        simplified_name = original_name
        if original_name.lower().endswith('.jpg.png'):
            simplified_name = original_name[:-8] + '.png'  # remove '.jpg.png' and append '.png'
        elif original_name.lower().endswith('.jpeg.png'):
            simplified_name = original_name[:-9] + '.png'
        else:
            # Ensure final extension is .png for mask output
            simplified_name = depth_path.stem + '.png'

        out_path = out_dir / simplified_name

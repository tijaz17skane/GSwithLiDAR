...
def main():
    ...
    # Prepare to append flat camera data if provided
    flat_cam_params = {}
    flat_sensor_to_camid = {}
    flat_next_cam_id = max(cam_params.keys(), default=0) + 1
    if args.calib_flat:
        with open(args.calib_flat, newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for i, row in enumerate(reader):
                sensor_id = row['sensor_id']
                cam_id = flat_next_cam_id
                flat_next_cam_id += 1
                fx = float(row['focal_length_px_x'])
                fy = float(row['focal_length_px_y'])
                cx = float(row['principal_point_px_x'])
                ...
                flat_sensor_to_camid[sensor_id] = cam_id
    ...
    with open(os.path.join(args.outdir, 'cameras.txt'), 'w') as f:
        ...
        for cam_id, params in flat_cam_params.items():
            f.write(f'{cam_id} {params["model"]} {params["width"]} {params["height"]} {params["fx"]} {params["fy"]} {params["cx"]} {params["cy"]}\n')
    ...
    # If meta_flat and calib_flat are provided, append their data
    if args.meta_flat and args.calib_flat:
        # Build a lookup for flat sensor_id to cam_id
        flat_sensor_to_camid = {v['sensor_id']: k for k, v in flat_cam_params.items()}
        with open(args.meta_flat) as f:
            meta_flat = json.load(f)
        for img in meta_flat['images']:
            sensor_id = img['sensor_id']
            cam_id = flat_sensor_to_camid.get(sensor_id)
            if cam_id is None:
                continue
            # No pose for flat images, so set dummy pose (identity)
            # If you have pose info, update this logic
            qw, qx, qy, qz = 1.0, 0.0, 0.0, 0.0
            tx, ty, tz = 0.0, 0.0, 0.0
            # Use next available id
            flat_id = len(image_data) + len(flat_image_data) + 1
            flat_image_data.append({
                'id': flat_id,
                'qw': qw, 'qx': qx, 'qy': qy, 'qz': qz,
                'tx': tx, 'ty': ty, 'tz': tz,
                'cam_id': cam_id,
            ...
            params = flat_cam_params[cam_id]
            flat_camera_positions.append(camera_to_training_format(
                len(camera_positions) + len(flat_camera_positions), qw, qx, qy, qz, 
                tx, ty, tz, params['width'], params['height'],
                params['fx'], params['fy'], cam_id, img["path"]
            ))
    
    # Write images.txt efficiently (concatenate with flat images if present)
    ...
    with open(os.path.join(args.outdir, 'images.txt'), 'w') as f:
        ...
        for img_data in flat_image_data:
            f.write(f'{img_data["id"]} {img_data["qw"]} {img_data["qx"]} {img_data["qy"]} {img_data["qz"]} {img_data["tx"]} {img_data["ty"]} {img_data["tz"]} {img_data["cam_id"]} {img_data["name"]}\n\n')
    
    # Write cameraPositions.txt (training format, concatenate with flat if present)
    ...
    with open(os.path.join(args.outdir, 'cameraPositions.txt'), 'w') as f:
        ...
        for cam in flat_camera_positions:
            tx, ty, tz = cam['position']
            rot = cam['rotation']
            f.write(f"{cam['id']} {tx:.6f} {ty:.6f} {tz:.6f} {rot[0][0]:.6f} {rot[0][1]:.6f} {rot[0][2]:.6f} {rot[1][0]:.6f} {rot[1][1]:.6f} {rot[1][2]:.6f} {rot[2][0]:.6f} {rot[2][1]:.6f} {rot[2][2]:.6f} {cam['cam_id']} {cam['img_name']}\n")

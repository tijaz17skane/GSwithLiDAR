#!/usr/bin/env python3
"""
Convert LAS/LAZ files to PLY format compatible with Gaussian Splatting
"""
...
def las_to_ply(las_path, ply_path, color_scale=1.0, intensity_to_rgb=True):
    """
    Convert LAS/LAZ file to PLY format
    Normalize XYZ coordinates by subtracting the minimum value along each axis
    """
    ...
    # ✅ Normalize XYZ by subtracting min along each axis
    min_xyz = xyz.min(axis=0)
    xyz -= min_xyz
    print(f"Normalized XYZ by subtracting min: {min_xyz}")
    ...
    print(f"Converted {len(xyz)} points to PLY format")
    print(f"Output saved to: {ply_path}")
    print(f"Point cloud bounds (after normalization):")
    print(f"  X: {xyz[:, 0].min():.3f} to {xyz[:, 0].max():.3f}")
    print(f"  Y: {xyz[:, 1].min():.3f} to {xyz[:, 1].max():.3f}")
    print(f"  Z: {xyz[:, 2].min():.3f} to {xyz[:, 2].max():.3f}")

def main():
    parser = argparse.ArgumentParser(description="Convert LAS/LAZ files to PLY format")
    ...
    try:
        las_to_ply(args.input, args.output, args.color_scale, not args.no_intensity_rgb)
